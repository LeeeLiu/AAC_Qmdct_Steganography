...
void loss_func(CoderInfo *coderInfo, int* xi, double* xr, double *pow43, double *xmin)
{
	int start,end, sb, i, j, sign;
	double f[FRAME_LEN], T[FRAME_LEN], sigmoid[FRAME_LEN];
	double invQuantFac, cost, costs[2 * FRAME_LEN], requant_xr[FRAME_LEN], diffvol, tmp,log;
	FILE * fp;
	fp = fopen("loss.xls", "a+");		
...
{	
    int sb, i, do_q = 0;
    int bits = 0, sign;
    double xr_pow[FRAME_LEN];//   |xr|^(3/4)
    double xmin[MAX_SCFAC_BANDS];
    int xi[FRAME_LEN]; //xi保存量化后的MDCT系数！
	...
	{				
        ...
        CalcAllowedDist(coderInfo, psyInfo, xr, xmin, aacquantCfg->quality);//计算Xmin(sfb)
		coderInfo->global_gain = 0;
		FixNoise(coderInfo, xr, xr_pow, xi, xmin,
				 aacquantCfg->pow43, aacquantCfg->adj43);//失真控制
		BalanceEnergy(coderInfo, xr, xi, aacquantCfg->pow43);
		UpdateRequant(coderInfo, xi, aacquantCfg->pow43);

        for ( i = 0; i < FRAME_LEN; i++ ) //加符号 
		{
            sign = (xr[i] < 0) ? -1 : 1;
            xi[i] *= sign;
	...
	/*计算失真(1)
	loss_func(coderInfo, xi, xr, aacquantCfg->pow43, xmin);//*/
	
	/*STC算出的stego,嵌入(2)
	FILE* fp;
	static int pos = 0;
	fp = fopen("Sqmdct.txt","r");
...
#endif
    // clamp to valid diff range比例因子带的差分编码
    {
...
//            printf("sf %d: %d diff=%d\n", i, coderInfo->scale_factor[i], diff);
	...
	static int jishu = 0;
	//记录
	if ((fp1 = fopen("stego_spec_data.txt", "a+")) == NULL){
...
#endif
/*
QuantizeBand(xr_pow, xi, IPOW20(coderInfo->global_gain),
             start, end, adj43);
*/
		    ...
		    double *adj43)
{
    ...
    double diffvol;
    double tmp;
    const double ifqstep = pow(2.0, 0.1875);// 2^(3/16)
